...
def get_top_movers(market: str = "KOSPI", n: int = 20) -> pd.DataFrame:
    ...
    df = stock.get_market_price_change(today, today, market=market)
...
def infer_sectors_from_movers() -> tuple[list[str], list[str]]:
    movers = pd.concat([
        get_top_movers("KOSPI", 15),
        get_top_movers("KOSDAQ", 15)
    ], ignore_index=True)
...
def polish_with_gpt(raw: dict) -> str:
    ...
    prompt = f"""
너는 한국 주식시장 요약 에디터다.
아래 JSON 데이터를 바탕으로 한국어 시장 요약문을 작성해라.
